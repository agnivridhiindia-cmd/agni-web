...
export function ServiceFaq({ faqs, serviceName }: ServiceFaqProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200/90 bg-white p-6 sm:p-7 shadow-xs">
        <Accordion type="single" collapsible className="w-full">
          {faqs.map((faq, index) => (
            <AccordionItem key={index} value={`faq-${index}`}>
              <AccordionTrigger className="text-left font-serif text-base sm:text-lg font-medium text-slate-900 hover:text-teal-700">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="text-sm text-slate-600 font-sans leading-relaxed">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
