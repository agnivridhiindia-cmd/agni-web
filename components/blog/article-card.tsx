...
export function ArticleCard({ post, featured = false, className }: ArticleCardProps) {
  ...
  if (featured) {
    return (
      <article
        className={cn(
          "group relative rounded-2xl border border-slate-300/90 bg-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:border-teal-500/50 overflow-hidden",
          className
        )}
      >
        {/* Top gold-to-teal metallic accent strip */}
        <div
          aria-hidden="true"
          className="h-1.5 w-full bg-gradient-to-r from-amber-500 via-teal-600 to-teal-400"
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch">
          ...
          <div className="lg:col-span-7 flex flex-col">
            <Link
              href={`/blog/${post.slug}`}
              className="block h-full group/banner focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-teal-600"
          ...
          <div className="lg:col-span-5 p-6 sm:p-8 lg:p-10 flex flex-col justify-between space-y-6 bg-white">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold uppercase tracking-wider text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
              ...
              <h2 className="font-serif font-bold text-2xl sm:text-3xl text-slate-900 tracking-tight leading-[1.25] group-hover:text-teal-700 transition-colors">
                <Link
                  href={`/blog/${post.slug}`}
            ...
            <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-full bg-teal-50 border border-teal-200/80 text-teal-700 flex items-center justify-center shrink-0">
              ...
              <Link
                href={`/blog/${post.slug}`}
                className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-teal-800 text-white font-semibold text-xs sm:text-sm hover:bg-teal-900 shadow-sm transition-all group/btn"
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col justify-between rounded-2xl border border-slate-200/90 bg-white shadow-xs hover:shadow-xl hover:border-teal-400 transition-all duration-300 overflow-hidden",
        className
      )}
    >
      ...
      <Link
        href={`/blog/${post.slug}`}
        className="block relative overflow-hidden focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-teal-600"
      ...
      <div className="p-6 sm:p-7 flex flex-col justify-between flex-1 space-y-4">
        <div className="space-y-3.5">
          ...
          {/* Title */}
          <h3 className="font-serif font-semibold text-xl text-slate-900 tracking-tight leading-snug group-hover:text-teal-700 transition-colors line-clamp-2">
            <Link
              href={`/blog/${post.slug}`}
              className="focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-teal-600 rounded"
            >
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
          ...
          <Link
            href={`/blog/${post.slug}`}
            className="inline-flex items-center gap-1 text-xs font-semibold text-teal-700 group-hover:text-teal-900 transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-teal-600 rounded px-1 py-0.5"
