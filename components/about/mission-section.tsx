...
export function MissionSection() {
  return (
    <section
      ...
      className="relative py-20 sm:py-24 lg:py-28 bg-slate-950 text-white border-b border-slate-800 overflow-hidden"
    >
      {/* Subtle Ambient Glow */}
      <div
      ...
      <Container width="wide" className="relative z-10 space-y-12 sm:space-y-16">
        ...
        <FadeIn direction="up" distance={20} delay={0.05}>
          <div className="max-w-4xl space-y-6">
            <span
              id="mission-heading"
              className="type-eyebrow text-gold-400 font-mono tracking-widest block uppercase"
            >
              Our Central Mission
            </span>

            <blockquote className="font-serif text-2xl sm:text-4xl lg:text-5xl font-normal leading-[1.25] text-slate-100 tracking-tight">
            ...
            <p className="type-body text-slate-400 max-w-2xl font-sans leading-relaxed">
        ...
        <FadeIn direction="up" distance={20} delay={0.15}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 border-t border-slate-800/80">
            {missionPillars.map((pillar) => (
              <div key={pillar.step} className="space-y-3">
                <span className="text-xs font-mono font-bold text-teal-400 block tracking-wider">
                  PHASE {pillar.step}
                </span>
          ...
          <div className="pt-8 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <div className="shrink-0">
              <LinkButton
                href="/services"
                variant="outline"
                className="text-slate-100 border-slate-700 bg-slate-900/80 hover:bg-slate-800 hover:border-slate-600 hover:text-white"
                aria-label="Explore all four advisory desks and services"
              >
                <span>Explore 4 Advisory Desks</span>
                <CtaArrow className="w-4 h-4 ml-2 text-teal-400" />
              </LinkButton>
            </div>
          </div>
        </FadeIn>
