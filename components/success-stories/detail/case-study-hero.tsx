  ...
  switch (category) {
    ...
    case "compliance":
      return {
        ...
        badgeClass: "bg-gold-50 border-gold-200 text-gold-900",
        icon: <ShieldCheck className="w-3.5 h-3.5 text-gold-700 shrink-0" aria-hidden="true" />,
      };
    case "digital":
      return {
        label: "Digital Transformation",
        badgeClass: "bg-slate-100 border-slate-200 text-slate-800",
      ...
      <Container width="wide">
        <div className="max-w-4xl space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
            ...
            {study.client && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium bg-white border border-slate-200 text-slate-700 shadow-xs">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-600" aria-hidden="true" />
                <span>{study.client}</span>
              </span>
            )}

            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-sans text-slate-500 bg-transparent">
          ...
          <h1
            ...
            className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-[3.25rem] font-semibold text-slate-900 tracking-tight leading-[1.18]"
          >
            {study.title}
          </h1>
